...
(function () {
  ...
  ready(function () {
    ...
    function go(n, fromRemote){
      ...
      // re-trigger entry animations on [data-anim] elements
      ...
      // re-trigger stagger-list animations (.anim-stagger-list and its children)
      slides[n].querySelectorAll('.anim-stagger-list').forEach(el => {
        el.classList.remove('anim-stagger-list');
        void el.offsetWidth;
        el.classList.add('anim-stagger-list');
      });

      // re-trigger any other anim-* classes on elements (fade-up, zoom-pop, etc.)
      slides[n].querySelectorAll('[class*="anim-"]').forEach(el => {
        const cls = Array.from(el.classList).filter(c => c.startsWith('anim-') && c !== 'anim-stagger-list');
        cls.forEach(c => {
